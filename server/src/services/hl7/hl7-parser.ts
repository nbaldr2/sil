export class HL7Parser {
  ...
  private parseMSH(fields: string[], message: any) {
    ...
    const messageType = fields[9].split(message.separators.component);
    message.type = messageType[0];
    message.triggerEvent = messageType[1];
    
    message.controlId = fields[10];
    message.processingId = fields[11];
    message.version = fields[12];
  }
  ...
  private parseComponentField(field: string) {
